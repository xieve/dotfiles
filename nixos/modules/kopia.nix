# Inspired by https://github.com/NixOS/nixpkgs/issues/163024#issuecomment-1822772284
...
let
  ...
  cfgFile = json.generate "kopia.json" (
    cfg.settings
    // {
      # "Token" (which is a config file encoded in base64 PLUS a password) parser wants this field
      version = "1";
    }
  );
        ...
        submodule {
          options = {
            ...
            server = mkOption {
              ...
              type = submodule {
                options = {
                  enable = mkEnableOption "Kopia Repository Server";
                  args = mkOption {
      ...
      systemd =
        ...
        mkMerge (
          map (
            directory:
            let
              serviceName = "kopia${systemdEscape directory}";
            in
            {
          ...
          ++ optional cfg.server.enable {
            services.kopia-server = commonConfig {
              ...
              type = "exec";
              command = ''
                server start ${cli.toCommandLineShellGNU { } (defaultServerArgs // cfg.server.args)}
              '';
            };
          }
        );
